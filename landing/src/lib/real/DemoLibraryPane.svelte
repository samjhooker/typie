<script>
  ...
  const transcripts = [
    { id:'t1', fileName:'Beta sync — Friday', date:'today', durationSeconds:724, speakerCount:2, isMeeting:true, preview:'and the checklist is done? two items — pricing and video', isNew:true },
    { id:'t2', fileName:'Quick voice note', date:'today', durationSeconds:5, speakerCount:1, isMeeting:false, preview:"don't forget to ship the beta", isNew:true },
    { id:'t3', fileName:'Team standup', date:'yesterday', durationSeconds:512, speakerCount:3, isMeeting:true, preview:'weekly sync — action items and blockers', isNew:false },
    { id:'t4', fileName:'Customer interview — Maya', date:'2 days ago', durationSeconds:1335, speakerCount:2, isMeeting:true, preview:'user research session about onboarding flow', isNew:false },
    { id:'t5', fileName:'Podcast recording draft', date:'3 days ago', durationSeconds:2710, speakerCount:2, isMeeting:false, preview:'the voice is the interface — discussing the future of input', isNew:false },
    { id:'t6', fileName:'Recipe from grandma', date:'4 days ago', durationSeconds:90, speakerCount:1, isMeeting:false, preview:"grandma's secret — add miso, white pepper, and a pinch of sugar", isNew:false },
  ]
...
<div class="wrap">
  ...
  <div class="actions">
    ...
    <div class="reccard">
      ...
      <p>saves the whole conversation offline — their side from your Mac's sound, yours mixed right in — then transcribes and splits the speakers.</p>
      <div class="acts">
  ...
  <div class="grid">
    {#each transcripts as item (item.id)}
      <button class="tcard card" onclick={onSelect}>
        ...
        <p class="meta">{item.date}{item.durationSeconds > 1 ? ` · ${fmtDur(item.durationSeconds)}` : ''}{item.speakerCount > 0 ? ` · ${item.speakerCount} spk` : ''}</p>
...
<style>
  ...
  .dz-hand{ font-size:15px; color:var(--text-3); transform:rotate(-1deg); display:inline-block }
  ...
  .rec-hand{ font-size:15px; color:var(--mint-live); transform:rotate(-2deg); display:inline-block }
  ...
  .search{
    ...
    max-width:260px; padding:8px 15px;
  ...
  .chip.new{ background:var(--card-mint); color:var(--green-deep); font-weight:700; padding:3px 10px; border-radius:999px; font-size:11px }
