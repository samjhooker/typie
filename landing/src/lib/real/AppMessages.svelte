<script>
  ...
  const convos = [
    ...
    { id: 2, name: 'Sam Baker', color: '#af52de', av: 'SB', time: '9:30', last: 'pricing page is live 🎉' },
    { id: 3, name: 'Design Team', color: '#007aff', av: '#', time: '9:12', last: 'Maya: shipped the hero' },
    { id: 4, name: 'Grandma', color: '#ff9500', av: 'G', time: 'Yesterday', last: 'add white pepper too' },
    { id: 5, name: 'Alex Rivera', color: '#30b0c7', av: 'AR', time: 'Yesterday', last: 'great meeting today' },
  ]
...
<div class="amsg">
  ...
  <section class="chat">
    <header>
      ...
      <div><b>Maya Chen</b><p>iMessage</p></div>
    ...
    <div class="bubbles">
      {#each thread as m}
        <div class="bub" class:mine={m.mine}>
          <p>{m.text}</p>
          <span class="tm">{m.time}{#if m.mine}<i>✓✓</i>{/if}</span>
        </div>
      {/each}
      {#if typed}
...
<style>
  .amsg{
    display:grid; grid-template-columns:255px 1fr; height:100%;
    background:#f6f6f6; font-family:'Inter',system-ui,sans-serif; color:#1c1c1e;
  }
  .list{
    background:#f2f2f7; border-right:1px solid rgba(0,0,0,.08);
    padding:10px 8px; display:flex; flex-direction:column; gap:2px; overflow:hidden;
  }
  .search{ font-size:11px; color:#8e8e93; background:rgba(120,120,128,.12); border-radius:7px; padding:5px 10px; margin-bottom:8px }
  .row{ display:flex; align-items:center; gap:9px; padding:6px 8px; border-radius:9px }
  .row.on{ background:rgba(0,0,0,.08) }
  .av{ flex:none; width:34px; height:34px; border-radius:50%; color:#fff; display:grid; place-items:center; font-size:11px; font-weight:700 }
  .av.sm{ width:28px; height:28px; font-size:10px }
  .meta{ min-width:0; flex:1 }
  .top{ display:flex; align-items:baseline; justify-content:space-between; gap:6px }
  .top b{ font-size:12.5px; font-weight:600 }
  .t{ font-size:10px; color:#8e8e93 }
  .last{ font-size:11px; color:#8e8e93; white-space:nowrap; overflow:hidden; text-overflow:ellipsis }
  ...
  header p{ font-size:10px; color:#8e8e93 }
  .facetime{ margin-left:auto; color:#007aff; font-size:12px }

  .bubbles{ flex:1; overflow:hidden; padding:14px; display:flex; flex-direction:column; gap:6px }
  .bub{
    align-self:flex-start; max-width:78%;
    background:rgba(255,255,255,.9); border-radius:16px 16px 16px 5px;
    padding:8px 11px; box-shadow:0 1px 2px rgba(0,0,0,.05);
  }
  .bub.mine{ align-self:flex-end; background:#0a7cff; color:#fff; border-radius:16px 16px 5px 16px }
  .bub p{ font-size:13px; line-height:1.4 }
  ...
  .field.dim{ color:#6d6d72; font-style:italic }
  .caret{ display:inline-block; width:2px; height:1em; margin-left:2px; vertical-align:-0.15em; background:#0b84fe; animation:blink .9s steps(1) infinite }
  @keyframes blink{ 50%{opacity:0} }
  .mic{ font-size:12px }

  @media (max-width:640px){
    .amsg{ grid-template-columns:1fr }
    .list{ display:none }
  }
</style>
