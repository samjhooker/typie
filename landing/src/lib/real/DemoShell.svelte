    ...
    <nav class:locked>
      {#each nav as item}
        <button
          ...
          class:disabled={locked && item.id !== startPane}
          title={item.label}
          onclick={() => go(item.id)}
...
<style>
  .shell {
    ...
    --ink: #131722;
    --text-1: var(--ink);
    --text-2: rgba(19, 23, 34, 0.76);
    --text-3: rgba(19, 23, 34, 0.54);
    --hotpink: #fc5681;
    --pink: #ffd3e0;
    --pink-band: #fbdae4;
    --green: #03594d;
    ...
    --violet-ink: #6a52c9;
    --red-ink: #c22e56;
    --glow-pink: rgba(252, 86, 129, 0.22);
    --glow-mint: rgba(130, 237, 166, 0.28);
    --line: rgba(3, 89, 77, 0.12);
  ...
  nav.locked .nav-item:not(.active) {
    opacity: 0.4;
    pointer-events: none;
    cursor: default;
  }
  ...
  .shell :global(.btn) {
    ...
    transition:
      ...
      box-shadow 0.35s var(--snap),
      color 0.25s var(--ease-out),
      border-color 0.2s var(--ease-out);
  }
  .shell :global(.btn:hover) {
    transform: translateY(-2px);
  }
  .shell :global(.btn:active) {
    transform: translateY(1px) scale(0.98);
    transition-duration: 0.12s;
  }
  .shell :global(.btn:disabled) {
    opacity: 0.45;
    cursor: default;
    transform: none;
  }

  .shell :global(.btn-pink) {
    background: var(--hotpink);
    color: #fff;
    box-shadow: 0 4px 14px rgba(252, 86, 129, 0.35);
  }
  .shell :global(.btn-pink:hover) {
    box-shadow: 0 7px 18px rgba(252, 86, 129, 0.42);
  }

  .shell :global(.btn-butter) {
    background: var(--butter);
    color: var(--ink);
    box-shadow: 0 4px 12px rgba(19, 23, 34, 0.12);
  }
  .shell :global(.btn-butter:hover) {
    background: var(--sun);
  }

  .shell :global(.btn-ghost) {
  ...
  .shell :global(.input:focus-within) {
    border-color: var(--hotpink);
    box-shadow: 0 0 0 3px rgba(252, 86, 129, 0.14);
  }
  .shell :global(.input input) {
    flex: 1;
    min-width: 0;
    background: none;
    border: none;
  ...
  .shell :global(.input input::placeholder) {
    color: var(--text-3);
  }
