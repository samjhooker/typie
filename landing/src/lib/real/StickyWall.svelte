<script>
  ...
  function look(id){ let h=0; for(const c of id) h=(h*31+c.charCodeAt(0))>>>0; return{ tint:tints[h%tints.length], rot:rots[h%rots.length] } }
...
<div class="wall">
  {#if extra}
    <article class="sticky fresh" style="background:{extra.tint.bg}; rotate:{extra.rot}" class:pinned={extra.pinned}>
...
<style>
  ...
  .sticky{
    ...
    transition:transform .22s var(--spring, ease), box-shadow .22s ease;
    cursor:pointer;
  }
  ...
  .txt{
    ...
    color:#131722; font-size:14px; line-height:1.55;
  }

  footer{ display:flex; align-items:center; justify-content:space-between }
  .meta{ font-family:'IBM Plex Mono', ui-monospace, monospace; font-size:10px; letter-spacing:.05em; text-transform:uppercase; color:rgba(19,23,34,.45) }

  .fresh{ animation:landIn .55s var(--spring, cubic-bezier(.22,1.2,.36,1)) both }
  @keyframes landIn{
    0%{ opacity:0; transform:translateY(26px) scale(.85) rotate(-6deg) }
    60%{ opacity:1; transform:translateY(-4px) scale(1.03) rotate(2deg) }
