...
<div class="wrap">
  ...
  <div class="wall">
    {#each notes as note (note.id)}
      ...
      <article
        class="sticky"
        class:fresh={extra && note.id === extra.id}
...
<style>
  ...
  .sticky.fresh {
    ...
    box-shadow:
      0 10px 28px rgba(252, 86, 129, 0.14),
      0 3px 12px rgba(19, 23, 34, 0.1);
    border: 1.5px solid rgba(252, 86, 129, 0.22);
  }
  @keyframes landIn {
    0% {
      opacity: 0;
      transform: translateY(28px) scale(0.82) rotate(-5deg);
      filter: blur(2px);
    }
    50% {
      opacity: 1;
