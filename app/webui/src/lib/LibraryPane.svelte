<script>
  ...
  const filtered = $derived.by(() => {
    ...
    if (query.trim()) {
      ...
      arr = arr.filter((x) =>
        `${x.fileName} ${x.preview ?? ''} ${(x.turns ?? []).map((t) => t.text).join(' ')}`
          .toLowerCase()
          .includes(q)
      );
    }
...
<div class="wrap">
  ...
  <div class="pane-host">
    {#if local.selectedTranscriptId}
      <div
        class="pane-view"
        in:fly={{ x: 48, duration: 340, easing: cubicOut }}
        out:fade={{ duration: 130 }}
      >
        <TranscriptDetail
          id={local.selectedTranscriptId}
          onBack={() => {
    ...
    {:else}
      <div
        class="pane-view"
        in:fly={{ x: -36, duration: 340, easing: cubicOut }}
        out:fade={{ duration: 130 }}
      >
        <header class="head">
          <div>
...
<style>
  ...
  .btn-stop:hover {
    box-shadow: 0 7px 18px rgba(194, 46, 86, 0.5);
  }
  .reccard p {
    font-size: 12.5px;
    color: var(--text-2);
