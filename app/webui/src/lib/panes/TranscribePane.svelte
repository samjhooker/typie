<script>
  ...
  const filtered = $derived(
    query.trim() === ''
      ? library
      : library.filter(x => `${x.fileName} ${(x.turns ?? []).map(t => t.text).join(' ')}`.toLowerCase().includes(query.trim().toLowerCase()))
  )
  ...
  function fmtDur(s){ const n = Math.round(s || 0); return `${Math.floor(n/60)}m ${String(n%60).padStart(2,'0')}s` }
...
<div class="wrap" class:embedded={embedded}>
  {#if !embedded}
    <header class="head">
      <div>
        <h2>Transcripts</h2>
        <p>drop any audio or video — diarized, timestamped, entirely on this Mac. <span class="hand hint-hand">like otter, but offline</span></p>
      </div>
    </header>
  {/if}
  ...
  {:else}
    ...
    >
      ...
      <h3>{hasWork ? 'add more files — they line up in the queue' : 'drop files, or click to browse'}</h3>
  ...
  <div class="lib-head">
    ...
    <div class="lib-tools">
      ...
      <label class="input search">
        <Search size={14} />
        <input bind:value={query} placeholder="search transcripts…" spellcheck="false" data-search />
      </label>
    </div>
  </div>
  ...
  {#if filtered.length === 0}
    <div class="empty">
      <span class="hand big">{library.length === 0 ? 'no transcripts yet — drop a file up there ☝' : `no matches for “${query}”`}</span>
    </div>
  {:else}
    <div class="grid">
      {#each visible as item (item.id)}
        <button class="tcard card" onclick={() => (local.selectedTranscriptId = item.id)}>
          <div class="top">
            <span class="ico" class:meeting={item.isMeeting}><Glyph name="transcript" size={16} /></span>
            {#if item.hasAudio}
          ...
          <p class="meta">{fmtDate(item.date)} · {fmtDur(item.durationSeconds)} · {item.speakerCount} speaker{item.speakerCount === 1 ? '' : 's'}{item.isMeeting ? ' · meeting' : ''}</p>
...
<style>
  ...
  .dropzone{
    ...
    cursor:pointer;
    transition:border-color .2s var(--ease-out), background .2s var(--ease-out), transform .2s var(--spring), opacity .2s var(--ease-out);
  }
  .dropzone.dim{ opacity:.75; min-height:140px }
  ...
  .bubble{
    ...
    box-shadow:0 8px 20px rgba(252,86,129,.35);
    margin-bottom:6px;
    transition:transform .25s var(--spring);
  }
  .dropzone:hover .bubble{ transform:scale(1.08) rotate(-4deg) }
  .dropzone h3{ font-size:19px }
  .note-hand{ font-size:18px; color:var(--mint-live); text-shadow:0 1px 0 rgba(255,255,255,.7); transform:rotate(-2deg); margin-top:8px }
  ...
  .count{
    font-family:var(--mono); font-size:11px; color:var(--text-3); vertical-align:2px; margin-left:4px;
  }
  .search{ max-width:280px; padding:8px 15px; background:var(--cream) }
  ...
  .chip.plain{ background:rgba(19,23,34,.06); color:var(--text-3) }
  .tcard h4{
