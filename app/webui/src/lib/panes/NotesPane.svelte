<script>
 ...
 import { markCopied } from '../bridge.svelte.js'
  ...
  const sorted = $derived.by(() => {
    ...
    switch (sortBy) {
      case 'oldest': return arr.sort((a, b) => pin(a) - pin(b) || new Date(a.date) - new Date(b.date))
      default:       return arr.sort((a, b) => pin(a) - pin(b) || new Date(b.date) - new Date(a.date))
    }
  })
  ...
  function fmtDur(s){ return s >= 60 ? `${Math.round(s/60)}m` : `${Math.round(s)}s` }

  function doNewNote(){ send({ type:'toggleNoteRecording' }) }
...
<div class="wrap">
  <header class="head">
    <div>
      <h2>Voice notes</h2>
      <p>hold <b>{ui.settings.hotkeyShort}</b>, speak — it lands here. <span class="hand hint-hand">your pocket for thoughts</span></p>
    </div>
    <div class="tools">
  ...
  {:else if sorted.length === 0}
    <div class="empty">
      <span class="hand big">{query.trim() ? `no matches for “${query}”` : 'the wall is empty…'}</span>
    ...
    <div class="wall">
      {#each visible as note (note.id)}
        ...
        <article class="sticky" style="background:{tint.bg}; rotate:{rot}" class:pinned={note.pinned}>
          ...
          <footer>
            <span class="meta">{fmtDateSmart(note.date)}{note.durationSeconds > 1 ? ` · ${fmtDur(note.durationSeconds)}` : ''}</span>
            <span class="acts">
              <button class="icon-btn" title="copy" onclick={() => { send({ type:'copy', text:note.text }); markCopied(note.id) }}>
                {#if local.copiedId === note.id}<span class="copied">✓</span>{:else}<Copy size={13} />{/if}
              </button>
              <button class="icon-btn" title={note.pinned ? 'unpin' : 'pin'} onclick={() => send({ type:'notesTogglePin', id:note.id })}>
...
<style>
  ...
  .search{ max-width:230px; padding:8px 15px; background:var(--cream) }

  .empty{
    padding:80px 20px; text-align:center;
    border:2px dashed var(--line-strong); border-radius:24px;
  }
  .big{ font-size:30px; color:var(--ink) }
  .empty p{ margin-top:10px; font-size:13px; color:var(--text-3) }
  ...
  .copied{ font-size:12px; color:var(--green); font-weight:800 }
